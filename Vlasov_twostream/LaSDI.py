...
class LaSDI:
    ...
    def __init__(self, encoder, decoder, NN = False, device = 'cpu', Local = False, Coef_interp = False, nearest_neigh = 4, Coef_interp_method = None, plot_fname = 'latent_space_dynamics.png'):
        ...
        if NN == False:
            ...
            import torch
            # Ensure dtype/device are always aligned with NN weights (float32),
            # even if callers pass float64 NumPy arrays.
            self.IC_gen = lambda IC: encoder(
                torch.as_tensor(IC, dtype=torch.float32, device=device)
            ).cpu().detach().numpy()
            self.decoder = lambda traj: decoder(
                torch.as_tensor(traj, dtype=torch.float32, device=device)
            ).cpu().detach().numpy()
